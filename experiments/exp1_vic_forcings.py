...
def main():
    ...
    with h5py.File(f"{args.datadir}/{args.forcing}.h5") as f:
        startdate = f.attrs["tstart"]
        enddate = f.attrs["tend"]
        bids = [b.decode() for b in f["bids"][:]]
    soilfile = f"{args.datadir}/vic/vic.nldas.mexico.soil.txt"
    outfile = f"{output_dir}/vic_{args.forcing}_baseline_soil.txt"
    with open(outfile, "w") as fout:
        for bid in tqdm(bids):
            line = vic.calibrate(bid, soilfile, args.forcing, startdate, enddate, datadir=args.datadir)
            fout.write(line)
    mod, obs = vic.evaluate(bids, outfile, args.forcing, startdate, enddate, args.datadir)
    ...
    val_tend = "2008-09-30"
    mod, obs = vic.evaluate(bids, outfile, args.forcing, val_tstart, val_tend, args.datadir)
